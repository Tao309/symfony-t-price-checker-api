...
class ImportProductPricesCommand extends CommonImportCommand
{
    ...
    protected function createEntityByImportRowData(array $row): ProductPrice
    {
        ...
        if (empty($row[self::FIELD_PRODUCT_ID])) {
            throw new \RuntimeException(sprintf('Поле product пустое для productPrice с userId = %s и dateCreated = %s', $row[self::FIELD_USER_CREATED_ID], $row[self::FIELD_DATE_CREATED]));
        }
        ...
        if (empty($row[self::FIELD_USER_CREATED_ID])) {
            throw new \RuntimeException(sprintf('Поле userCreated пустое для productPrice с productId = %s и dateCreated = %s', $row[self::FIELD_PRODUCT_ID], $row[self::FIELD_DATE_CREATED]));
        }

        $user = $this->userRepository->find($row[self::FIELD_USER_CREATED_ID]);

        if (empty($user)) {
            throw new \RuntimeException(sprintf('Не найден userCreated %s для productPrice с productId = %s и dateCreated = %s', $row[self::FIELD_USER_CREATED_ID], $row[self::FIELD_PRODUCT_ID], $row[self::FIELD_DATE_CREATED]));
        }
