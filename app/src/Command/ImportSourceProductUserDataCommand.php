...
class ImportSourceProductUserDataCommand extends CommonImportCommand
{
    ...
    protected function fillImportRow(array $row): void
    {
        $this->importData[] = [
            ...
            self::FIELD_LISTEN_PRICE_VALUE => $row[2] ? (int) $row[2] : null,
            self::FIELD_COMMENT => $row[3] ? trim($row[3]) : null,
    ...
    protected function createEntityByImportRowData(array $row): SourceProductUserData
    {
        ...
        if (empty($row[self::FIELD_SOURCE_PRODUCT_ID])) {
            throw new \RuntimeException(
                \sprintf(
                    'Поле product пустое для SourceProductUserData с userId = %s',
        ...
        if (empty($row[self::FIELD_USER_CREATED_ID])) {
            throw new \RuntimeException(
                \sprintf(
                    'Поле userCreated пустое для SourceProductUserData с sourceProductId = %s и dateCreated = %s',
                    $row[self::FIELD_SOURCE_PRODUCT_ID],
                    $row[self::FIELD_DATE_CREATED]
                )
            );
        ...
        if (empty($user)) {
            throw new \RuntimeException(
                \sprintf(
                    'Не найден userCreated %s для SourceProductUserData с sourceProductId = %s и dateCreated = %s',
                    $row[self::FIELD_USER_CREATED_ID],
                    $row[self::FIELD_SOURCE_PRODUCT_ID],
                    $row[self::FIELD_DATE_CREATED]
                )
