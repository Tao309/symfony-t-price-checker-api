...
class SourceProductRepository extends ServiceEntityRepository
{
    ...
    public function findWithPagination(int $page = 1, int $itemsPerPage = 30, ?string $title = null): DoctrinePaginator
    {
        ...
        if ($title) {
            ...
            $qb
                ->where('LOWER(sp.title) = :title')
                ->orWhere('LOWER(sp.title) LIKE :like_title')
            ...
            $qb
                ->addSelect(implode(' ', [
                    '(CASE',
                    'WHEN sp.title = :o_title_exac THEN 1',
                    'WHEN sp.title LIKE :o_title_right THEN 2',
                    'WHEN sp.title LIKE :o_title_twice THEN 3',
                    'WHEN sp.title LIKE :o_title_left  THEN 4',
                    'ELSE 5',
            ...
            if (str_contains($title, ':')) {
                ...
                $qb
                    ->orWhere('LOWER(sp.title) LIKE :title_split_colon')
            ...
            if (str_contains($title, '.')) {
                ...
                $qb
                    ->orWhere('LOWER(sp.title) LIKE :title_split_dot')
            ...
            if (str_contains($title, ' ')) {
                ...
                $qb
                    ->orWhere('LOWER(sp.title) LIKE :title_split_space')
